...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    setState(() {
      ...
      if (userRole == "student") {
        selectedGrade = prefs.getString("grade");
  ...
  Future<void> _saveProfile() async {
    ...
    if (userRole == "student" &&
      ...
      _showErrorDialog("Please select your Grade and Section.");
      return;
    }
    ...
    // If username changed, we need to migrate the display name
    if (usernameChanged) {
      String? oldDisplayName = prefs.getString(
        "display_name_${widget.currentUsername}",
      );

      // Remove old display name key
      await prefs.remove("display_name_${widget.currentUsername}");

      // Save display name with new username key
      await prefs.setString(
        "display_name_$newUsername",
        _displayNameController.text.trim(),
      );
    } else {
      // Just update the display name with current username
      await prefs.setString(
        "display_name_${widget.currentUsername}",
        _displayNameController.text.trim(),
      );
    ...
    // Pop back to settings with updated username if it changed
